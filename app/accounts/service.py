...
class LedgerService:
    ...
    @staticmethod
    def get_account_entries(db: Session, account_id: int):
...
class AutoPostingService:
    ...
    @staticmethod
    def post_purchase(db: Session, purchase):

        inventory = (
            db.query(Account).filter(Account.account_name == "Inventory").first()
        )

        purchase_account = (
            db.query(Account).filter(Account.account_name == "Purchase").first()
        )

        supplier = db.query(Account).filter(Account.account_name == "Supplier").first()
    ...
    @staticmethod
    def post_sales(db: Session, invoice):

        sales = db.query(Account).filter(Account.account_name == "Sales").first()

        customer = db.query(Account).filter(Account.account_name == "Customer").first()

        gst = db.query(Account).filter(Account.account_name == "GST Output").first()
...
class PaymentPostingService:
    ...
    @staticmethod
    def supplier_payment(db, payment):

        supplier = db.query(Account).filter(Account.account_name == "Supplier").first()

        cash = db.query(Account).filter(Account.account_name == "Cash").first()
...
class ReceiptPostingService:
    ...
    @staticmethod
    def customer_receipt(db, receipt):
