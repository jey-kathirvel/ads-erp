...
async def save_invoice(
    ...
    for i in range(len(product_id)):
        ...
        product = ProductService.get_by_id(db, product_id[i])
        ...
        ProductService.update_stock(db, product_id[i], qty[i])

    return RedirectResponse(url=f"/billing/view/{saved_invoice.id}", status_code=303)
...
@router.get("/billing/list", response_class=HTMLResponse)
async def invoice_list(request: Request, db: Session = Depends(get_db)):

    invoices = BillingService.get_all(db)

    return templates.TemplateResponse(
        request=request, name="billing/list.html", context={"invoices": invoices}
    )
...
@router.get("/billing/view/{invoice_id}", response_class=HTMLResponse)
async def view_invoice(
    ...
    items = BillingService.get_items(db, invoice_id)
...
@router.get("/billing/{invoice_id}/edit", response_class=HTMLResponse)
async def edit_invoice(
    ...
    if invoice is None:
        ...
        return RedirectResponse("/billing/list", status_code=303)
...
@router.post("/billing/{invoice_id}/edit")
async def update_invoice(
    ...
    return RedirectResponse(url=f"/billing/view/{invoice_id}", status_code=303)
...
@router.get("/billing/{invoice_id}/delete")
async def delete_invoice(invoice_id: int, db: Session = Depends(get_db)):

    BillingService.delete(db, invoice_id)
...
@router.get("/billing/print/{invoice_id}", response_class=HTMLResponse)
