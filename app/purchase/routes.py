...
async def save_purchase(
    ...
    saved_purchase = PurchaseService.create(db, purchase)

    # ---------------------------------
    # Auto Ledger Posting
    # ---------------------------------

    AutoPostingService.post_purchase(db, saved_purchase)
    ...
    return RedirectResponse(url=f"/purchase/view/{saved_purchase.id}", status_code=303)
...
@router.get("/purchase/list", response_class=HTMLResponse)
async def purchase_list(request: Request, db: Session = Depends(get_db)):

    purchases = PurchaseService.get_all(db)

    return templates.TemplateResponse(
        request=request, name="purchase/list.html", context={"purchases": purchases}
    )
...
@router.get("/purchase/delete/{purchase_id}")
async def delete_purchase(purchase_id: int, db: Session = Depends(get_db)):

    PurchaseItemService.rollback_stock(db, purchase_id)

    PurchaseItemService.delete_items(db, purchase_id)

    PurchaseService.delete(db, purchase_id)

    return RedirectResponse(url="/purchase/list", status_code=303)
